...
__global__ void scan(
    ...
    int offset_gate = (kNChunksPerSequence * 256 * blockIdx.z);
    int offset_out = (blockIdx.x * batch_stride_o) + (blockIdx.y * 64) + (blockIdx.z * 4);
    ...
    for (int i = 0; i < kNChunksPerSequence; ++i)
    {
        ...
        weight_t x = tokens[offset + threadIdx.x] * gates[offset_gate + threadIdx.x + (i * blockDim.x)];
        odd = lane_id & 1;
...
warpscan(const at::Tensor &gates, const at::Tensor &tokens, const at::Tensor &out, const bool reverse) {
    ...
    dim3 grid(batch_size, dim, out.strides()[1] / 4); // 4 is sequential steps;
